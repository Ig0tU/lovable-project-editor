...
// NLP Processor for Chrome Extension
export class NLPProcessor {
  ...
  async processCommand(command) {
    ...
    for (const [action, patterns] of Object.entries(this.patterns)) {
      for (const pattern of patterns) {
        ...
        if (match) {
          return this.buildIntent(action, match, command);
        }
      }
    }
  ...
  buildIntent(action, match, originalCommand) {
    const intent = { action: action, target: '' };
  ...
  parseStyleProperties(styleStr, fullCommand) {
    const properties = {};
